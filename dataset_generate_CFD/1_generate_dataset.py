...
# ================= 配置区域 =================
DATASET_SIZE = 600           # 生成样本数量
RESOLUTION = 120            # 分辨率设为 120
OUTPUT_DIR = "dataset_ml"   # 输出文件夹
...
def generate_random_inputs():
    ...
    # 2. 随机密度网格 (3, 3, 3)
    # 离散值: [0.3, 0.4, 0.5]
    # 动态期望: 在 0.3 到 0.4 之间随机选择一个目标期望
    density_choices = [0.3, 0.4, 0.5]
    
    # 混合两个基准分布来实现动态期望
    # Dist_A (Mean ≈ 0.32): [0.85, 0.1, 0.05]
    # Dist_B (Mean ≈ 0.40): [0.1, 0.8, 0.1]
    alpha = np.random.rand() # 0~1 随机系数
    probs = (1 - alpha) * np.array([0.85, 0.1, 0.05]) + alpha * np.array([0.1, 0.8, 0.1])
    probs /= probs.sum() # Normalize to ensure sum is exactly 1.0
    ...
    for axis in range(3): # 对Rx, Ry, Rz三个分量分别生成
        base_val = np.random.uniform(0, 180)
        # 随机生成每个方向的梯度，范围 [-15, 15]
        # 这样任意相邻网格的差值最大为 15
        d_ang = np.random.uniform(-15, 15, size=3)
        
        # 计算线性场: V = Base + x*dx + y*dy + z*dz
        vals = base_val + xx * d_ang[0] + yy * d_ang[1] + zz * d_ang[2]
        
        # 截断到 [0, 180] 范围
        # 截断操作不会增加相邻点的差值，因此梯度约束仍然满足
        vals = np.clip(vals, 0, 180)
